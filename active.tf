resource "azurerm_linux_virtual_machine" "activefgtvm" {
  ...
  source_image_reference {
    ...
    publisher = "fortinet"
    sku       = "fortinet_fg-vm"
    version   = "7.2.8" //To validate the latest version. "az vm image list --offer fortinet_fortigate-vm_v5 --publisher fortinet --all --query "[].{Sku:sku, Version:version}" --output table"
  }
...
resource "azurerm_managed_disk" "res-1" {
  create_option        = "Empty"
  location             = var.location
  name                 = "${local.customer_prefix}_active_disk2"
  resource_group_name  = azurerm_resource_group.res-0.name
